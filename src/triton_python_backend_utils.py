...
def deserialize_bytes_tensor(encoded_tensor: bytes) -> np.array:
    """
...
def get_input_tensor_by_name(inference_request, name: str):
...
def get_output_tensor_by_name(inference_response, name: str):
...
def get_output_config_by_name(model_config: dict, name: dict) -> dict:
...
def triton_to_numpy_type(data_type: int):
    if data_type == 1:
        return np.bool_
    elif data_type == 2:
        return np.uint8
...
def numpy_to_triton_type(data_type) -> int:
...
def triton_string_to_numpy(triton_type_string: str):
